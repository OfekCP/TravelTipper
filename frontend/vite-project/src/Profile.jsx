...
const Profile = (setExperienceId) => {
  ...
  const fetchUserExperiences = async () => {
    try {
      const experiencesResponse = await axios.get('/api/travel/experiences');
  ...
  const fetchFriendRequests = async () => {
    try {
      const response = await axios.get(`/auth/users/friend-requests`);
  ...
  const handleFileChange = async (event) => {
    ...
    const url = `/auth/users/profile-picture`;
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.post('/auth/user/update', updateData);
      setEditMode(false);
      setUser({
        ...user,
        username: formData.username,
  ...
  const handleDelete = async (experienceId) => {
    try {
      await axios.delete(`/api/travel/experiences/${experienceId}`)
      fetchUserExperiences();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleExperienceUpdate = async (event, experienceId) => {
    ...
    try {
      ...
      await axios.put(`api/travel/experiences/${experienceId}`, formData);
  ...
  const handleRemoveProfilePicture = async () => {
    try {
      await axios.delete(`/auth/users/profile-picture`);
  ...
  const handleAcceptFriendRequest = async (senderId) => {
    try {
      const response = await axios.post(`auth/users/${senderId}/accept-friend-request`);
  ...
  const handleDeclineFriendRequest = async (senderId) => {
    try {
      const response = await axios.post(`auth/users/${senderId}/decline-friend-request`);
      console.log(response.data.message); 
     fetchFriendRequests();
    } catch (error) {
  ...
  const handleSaveBio = async () => {
    try {
      if(editBio){
        const response=await axios.put('auth/users/editBio',{bio:bio})
        setBio(response.data.bio)
        fetchUser();
      }
      const response=await axios.post('auth/users/createBio',{bio:bio})
      setBio(response.data.bio)
      setEditBio(false);
      fetchUser();
    } catch (error) {
